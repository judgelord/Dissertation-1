...
The time range of the data is important. Over this period of time, the level of military prestige changes from one that was moderate to one that was relatively high. Simultaneously, the level of political polarization changes from one that was also relatively moderate to one that is relatively high. To better grasp these changes, consider that in 1979, only 54% of Americans surveyed expressed either a "great deal" or "quite a lot" of trust in the nation's military [@noauthor_confidence_2020]. In 2020, 72% of those surveyed expressed the same [@noauthor_confidence_2020]. Since public opinion data has been kept on this question going back to 1975, the lowest and highest percentage of the American population expressing these levels of confidence in the nation's military were 50% (1981) and 85% (February of 1991)[@noauthor_confidence_2020].^[1991 is the only year in which two data points are given. The level of confidence dropped to 69% by October of the same year, suggesting that the February data point captured an effect related to the Gulf War, which ended in early 1991.] The levels of trust in the military and polarization from 1979 - 2020 are displayed in Figure \@ref(fig:polar-trust-oped). 
...
df_trust_polar_ch3 <- df_trust_polar_ch3 %>% select(-Congress) %>% filter(Year<=2020) %>% 
  mutate(
    ...
    ) %>% pivot_longer(Military:sen_polar_dim1, names_to = "value", values_to = "Score") %>% distinct() %>% na.omit() %>% view()
```
...
ggplot(data=violation_reg_df, mapping=aes(x=PubYr, y=Prop_Pubs_Viol)) + geom_point() + geom_smooth() + labs(x="Year", y="Proportion", color=NULL) + geom_vline(xintercept = 1981, size=.2, colour="tomato") + 
  geom_vline(xintercept=1989, size=.2, colour="tomato") + 
  geom_vline(xintercept=1993, size=.2, colour="dodgerblue") + geom_vline(xintercept=2001, size=.2, colour="tomato") + 
  geom_vline(xintercept=2009, size=.2, colour="dodgerblue") +
  geom_vline(xintercept=2017, size=.2, colour="tomato") +
...
```{r pred-prob-sen, include=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Predicted Probability of Senate Polarization\n on Retired Military Elite Opinion Publications Violating Principles of Civil-Military Relations", fig.height=4.5, fig.width=6}
predictions%>%
  ggplot() +
...
predictionshouse%>%
  ...
  geom_vline(xintercept=70.43, size=.2, colour="dodgerblue") +
  geom_vline(xintercept=78.66, size=.2, colour="tomato") + 
  geom_vline(xintercept=80.45, size=.2, colour="dodgerblue") +
  geom_vline(xintercept=86.14, size=.2, colour="dodgerblue") +
  geom_vline(xintercept=88.37, size=.2, colour="dodgerblue") +
  geom_text(aes(x=59.6, label="1979",y=.5), colour="dodgerblue", angle=90, vjust=1, text=element_text(size=9)) + 
  geom_text(aes(x=70.43, label="1993",y=.5), colour="dodgerblue", angle=90, vjust=1, text=element_text(size=9)) + 
  ...
  geom_text(aes(x=86.14, label="2011",y=.65), colour="dodgerblue", angle=90, vjust=-.3, text=element_text(size=9))  + 
  geom_text(aes(x=88.37, label="2021", y=.65), colour="dodgerblue", angle=90, vjust=1.1, text=element_text(size=9))
```
